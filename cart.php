...
if (isset($_GET['id'], $_GET['action'], $_GET['size'])) {
    ...
    if ($transactionQuery->num_rows > 0) {
        ...
        if ($action === 'add') {
            if (isset($_SESSION['cart'][$compositeKey])) {
                // Fetch available stock from the stock table
                $stockQuery = $conn->query("SELECT qty FROM stock WHERE product_id = $productId AND product_size = '$size'") or die(mysqli_error($conn));
                if ($stockQuery->num_rows > 0) {
                    $stock = $stockQuery->fetch_assoc();
                    $availableStock = (int)$stock['qty'];

                    // Check if the cart quantity has reached the stock limit
                    if ($_SESSION['cart'][$compositeKey]['quantity'] < $availableStock) {
                        ...
                        echo "<script>alert('Sorry, you cannot add more of this product. Available stock: $availableStock');</script>";
                        echo "<script>window.location.href = 'cart.php';</script>";
                        exit;
                    }
                } else {
                    echo "<script>alert('Stock information not found for this product and size.');</script>";
                    echo "<script>window.location.href = 'cart.php';</script>";
                    exit;
                }
            }
