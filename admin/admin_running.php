...
if (isset($_POST['add'])) {
    ...
    usort($product_size, function($a, $b) {
        return (float)filter_var($a, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION) 
               <=> 
               (float)filter_var($b, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
    });
    ...
    if ($error > 0) {
        ...
        if ($size > 30000000000) {
            ...
            $conn->query("INSERT INTO product (product_id, product_name, product_price, product_image, brand, category, product_size)
                          VALUES ('$product_code', '$product_name', '$product_price', '$name', '$brand', '$category', '$product_size_str')");
            ...
            foreach ($sizes as $size) {
                if (in_array($size, $product_size)) {
                    $sanitized_size = str_replace([' ', '.'], '_', $size); // Sanitize size for the quantity key
                    $qty_key = 'qty_' . $sanitized_size; // e.g., qty_US_7_5
                    $qty = isset($_POST[$qty_key]) ? $_POST[$qty_key] : 0; // Default to 0 if quantity is not provided

                    // Insert into stock table
                    $conn->query("INSERT INTO stock (product_id, product_size, qty)
                                  VALUES ('$product_code', '$size', '$qty')") or die(mysqli_error($conn));
                }
            }
...
<body style="display:unset;">
    <div id="header" style="position:fixed;">
        ...
        ?>
       <ul>
			 <li><a href="admin_index.php"><i class="icon-off icon-white"></i> Logout</a></li>
        ...
        <ul>
            ...
            <li><a href="admin_home.php">Products</a>
                <ul>
                    ...
                    <li><a href="admin_running.php "style="font-size:15px; margin-left:15px;">Basketball</a></li>
    ...
    <div id="rightcontent">
        ...
        <table class="table table-hover" style="background-color:;">
            ...
            <tr class="del<?php echo $id?>">
                ...
                <td>
                    <?php
                    // Fetch and display sizes for the product
                    $size_query = $conn->query("SELECT product_size FROM `stock` WHERE product_id='$id'") or die(mysqli_error());
                    $sizes = [];
                    while ($size_fetch = $size_query->fetch_array()) {
                        $sizes[] = $size_fetch['product_size'];
                    }
                    echo implode(", ", $sizes); // Display sizes as a comma-separated list
                    ?>
                </td>
    ...
    <div style="display: flex; gap: 5px; align-items: center;">
        ...
        <a href='admin_running.php?delete_id=<?php echo $id; ?>' 
   ...
   if (isset($_POST['stockin'])) {
    ...
    // Sort the selected sizes in ascending order
    usort($selected_sizes, function($a, $b) {
        return (float)filter_var($a, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION) 
               <=> 
               (float)filter_var($b, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
    });
    ...
    // Sort all sizes in ascending order
    ...
    echo "<script>
            ...
            window.location = 'admin_running.php';
          </script>";
    exit();
}
