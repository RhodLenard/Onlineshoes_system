document.addEventListener('DOMContentLoaded', function () {
  ...
  function enableDarkMode() {
    ...
    // ✅ Apply darker background and text color for the entire body
    body.style.backgroundColor = '#2c2c2c'; // A soft dark gray
    body.style.color = '#dcdcdc'; // Light gray text

    // ✅ Modify all links to be softer in color
    document
      .querySelectorAll('a')
      .forEach((link) => (link.style.color = '#b0b0b0'));
    ...
    // ✅ Darken content divs
    document
      .querySelectorAll('.content, #container, #content')
      .forEach((div) => {
        div.style.backgroundColor = '#333333'; // Dark background for content divs
        div.style.color = '#dcdcdc'; // Light gray text for content divs
      });

    // ✅ Darken form inputs, textareas, and selects
    document.querySelectorAll('input, textarea, select').forEach((input) => {
      input.style.backgroundColor = '#555555'; // Dark background for inputs
      input.style.color = '#dcdcdc'; // Light text color for inputs
      input.style.border = '1px solid #444444'; // Dark border
    });

    // ✅ Modify labels for form fields
    document.querySelectorAll('label').forEach((label) => {
      label.style.color = '#dcdcdc'; // Light text color for labels
    });
  ...
  function disableDarkMode() {
    ...
    // ✅ Reset content div styles
    document
      .querySelectorAll('.content, #container, #content')
      .forEach((div) => {
        div.style.backgroundColor = ''; // Reset content div background
        div.style.color = ''; // Reset content div text color
      });

    // ✅ Reset form inputs, textareas, and selects
    document.querySelectorAll('input, textarea, select').forEach((input) => {
